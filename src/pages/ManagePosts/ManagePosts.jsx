...
export function ListPostOwn({ user }) {
  ...
  const handleDelete = async (id) => {
    ...
    if (isBuying) {
      toast.error(
        "No es posible eliminar este elemento, solo desactivar su publicacion"
      );
      return;
    }
    deletePost(id)
  ...
  const changeStatePost = (post) => {
    let inventory = post.inventory;

    if (inventory > 0) {
      updateStatePost(post.id, post.state ? !post.state : true);
    } else {
      setSelectPost(post);
      setOpenModal(true);
    }
    ...
    <>
      ...
      {empty && (
        <div className="container-center-item">
          <span>
            No hay publicaciones <FaShoppingCart className="inline-block" />
          </span>{" "}
        </div>
      )}
      {loading && (
...
export function FormPost(props) {
  ...
  const formik = useFormik({
    initialValues: { id, inventory: inventory ?? 1 },
    validationSchema: yup.object({
      inventory: yup.number().required("Se requiere un valor").min(1,"Se requiere al menos uno").max(15,"Solamente se puede tener 15 en inventario"),
    }),
    onSubmit: (data) => {
