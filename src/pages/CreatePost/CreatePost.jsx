...
const CreatePost = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      description: yup
        ...
        .min(5, "Se requiere una descripcion mas completa")
        .max(500, "Solo se pueden escribir 500 caracteres"),
      images: yup
        .array()
        .of(yup.string())
      ...
      precio: yup
        .number()
        .required("Coloca un precio")
        .positive("El precio debe ser mayor a $ 0.00"),
      inventory: yup
        .number()
        .positive()
        .min(1, "Debe almenos tener 1 en inventario")
        .required("Se requiere una cantidad en inventario"),
      address: yup
        .string()
        .required("Se requiere el la direccion")
        .min(20, "Se requiere una direccion mas completa"),
    }),
    onSubmit: (values) => {
        ...
        uploadImagesStorage(values.images, docRef.id)
          .then((urlImages) =>
            setDoc(docRef, {
              ...
              images: urlImages,
              address: `En Tec Saltillo; ${values.address}`,
          ...
          .then(() => {
            toast.success("Se ha publicado el producto", {
  ...
  const uploadImagesStorage = async (files, docRef) => {
    ...
    return new Promise((resolve, reject) => {
      urlsImages.length > 0 ? resolve(urlsImages) : reject(null);
    });
  };
      ...
      <div className="max-w-screen-xl px-4 py-16 mx-auto sm:px-6 lg:px-8 bg-primary">
        ...
        <div className="max-w-lg mx-auto">
          ...
          >
                  ...
                  >
                    {formik.values.images.length > 0 ? (
                      <div className="relative w-full mx-auto flex flex-row gap-6 snap-x snap-mandatory overflow-x-auto pb-8 rounded-lg">
                        ...
                        {Array.from({
                          length: formik.values.images.length,
                        }).map((_, index) => (
                          <div
                          ...
                          >
                            <img
                              ...
                              src={formik.values.images[index]}
                              alt={`upload_${index}`}
                            />
                          </div>
                        ))}
            ...
            <div id="price" className="relative mt-1 flex items-center">
              <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                $
              </span>
              <input
                type="number"
                className="w-full border-2 pl-10 border-gray-200 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                placeholder="Precio"
                id="precio"
                name="precio"
            ...
            <div id="invenotry-input">
              ...
              <div className="relative mt-1">
                ...
                {formik.touched.inventory &&
                  Boolean(formik.errors.inventory) && (
                    <div className="font-semibold text-primary">
